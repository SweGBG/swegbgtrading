...
export default function AdminPage() {
  ...
  const saveProduct = async () => {
    ...
    const isNew = editingProduct.id === -1;
      ...
      >
        ...
        <div style={{ display: "flex", gap: "32px", flexWrap: "wrap", alignItems: "flex-start" }}>
            ...
            {tabs.map((tab, i) => (
              <motion.button key={tab.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 + i * 0.07 }}
                ...
                style={{
                  ...
                  borderRadius: "0 6px 6px 0", cursor: "pointer", fontSize: "13px", letterSpacing: "1px", transition: "all 0.2s",
                }}
              >
                {tab.label}
              </motion.button>
          ...
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
            style={{ flex: 1, minWidth: "280px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "16px", padding: "40px" }}
          >
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                    {orders.map((order, i) => (
                      <motion.div key={order.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                        style={{ padding: "20px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "10px" }}
                      >
                            ...
                            {STATUS_OPTIONS.map((status) => (
                              <button key={status} onClick={() => updateOrderStatus(order.id, status)} disabled={updatingOrder === order.id}
                                style={{
                                  ...
                                  fontSize: "11px", letterSpacing: "1px", cursor: "pointer", transition: "all 0.2s",
                                }}
                              >
                                {status}
                              </button>
                ...
                <motion.div key="products" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "28px" }}>
                    ...
                    {!editingProduct && (
                      <button onClick={() => setEditingProduct({ ...EMPTY_PRODUCT })}
                        style={{ padding: "10px 20px", background: "rgba(180,140,60,0.9)", border: "none", borderRadius: "8px", color: "#000", fontSize: "11px", fontWeight: "800", letterSpacing: "2px", textTransform: "uppercase", cursor: "pointer" }}
                      >
                        + Ny produkt
                      </button>
                    )}
                  ...
                  {editingProduct ? (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                        <div>
                          ...
                          <input type="text" value={editingProduct.Name} placeholder="GBG Brew"
                        ...
                        <div style={{ display: "flex", gap: "12px" }}>
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            <input type="text" value={editingProduct.emoji} placeholder="☕"
                        ...
                        <div style={{ display: "flex", gap: "12px" }}>
                          <div style={{ flex: 1 }}>
                            ...
                            <input type="text" value={editingProduct.badge} placeholder="new"
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            <input type="text" value={editingProduct.category} placeholder="Kaffe"
                        ...
                        <div>
                          ...
                          <textarea value={editingProduct.description} placeholder="Beskriv produkten..."
                        ...
                        >
                          {savingProduct ? "Sparar..." : editingProduct.id === -1 ? "Skapa produkt" : "Spara ändringar"}
                        </button>
                      </div>
                    </motion.div>
                  ) : (
                    <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                      {products.length === 0 && (
                        <p style={{ color: "rgba(255,255,255,0.25)", fontSize: "13px", letterSpacing: "2px" }}>Inga produkter — klicka "+ Ny produkt" för att lägga till.</p>
                      )}
                      {products.map((product, i) => (
                        <motion.div key={product.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}
                          style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "18px 20px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "10px", gap: "12px", flexWrap: "wrap" }}
                        >
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                            <button onClick={() => setEditingProduct(product)}
                              style={{ padding: "7px 14px", background: "rgba(180,140,60,0.1)", border: "1px solid rgba(180,140,60,0.3)", borderRadius: "6px", color: "rgba(180,140,60,0.8)", fontSize: "11px", letterSpacing: "2px", textTransform: "uppercase", cursor: "pointer" }}
                            >
                              Redigera
                            </button>
                            <button onClick={() => deleteProduct(product.id)}
                              style={{ padding: "7px 14px", background: "rgba(255,80,80,0.08)", border: "1px solid rgba(255,80,80,0.2)", borderRadius: "6px", color: "rgba(255,80,80,0.6)", fontSize: "11px", letterSpacing: "2px", textTransform: "uppercase", cursor: "pointer" }}
                            >
                              Ta bort
                            </button>
                          </div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                    {messages.length > 0 ? messages.map((msg, i) => (
                      <motion.div key={msg.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                        style={{ padding: "16px 20px", background: msg.read ? "rgba(255,255,255,0.02)" : "rgba(180,140,60,0.06)", border: `1px solid ${msg.read ? "rgba(255,255,255,0.05)" : "rgba(180,140,60,0.2)"}`, borderRadius: "8px" }}
                      >
