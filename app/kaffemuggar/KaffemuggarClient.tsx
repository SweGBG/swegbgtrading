...
export default function KaffemuggarClient() {
  return (
    <main style={{ minHeight: "100vh", background: "#0a0a0a", overflow: "hidden" }}>
      ...
      <div style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none", background: "radial-gradient(ellipse at 50% 30%, rgba(180,140,60,0.08) 0%, transparent 60%)" }} />
      ...
      >
        ...
        <div style={{ position: "relative", width: "120px", height: "120px", marginBottom: "60px" }}>
          {[0, 1, 2].map((i) => (
            <motion.div key={i} animate={{ scale: [1, 1.3, 1], opacity: [0.3, 0, 0.3] }} transition={{ duration: 3, repeat: Infinity, delay: i * 1, ease: "easeInOut" }} style={{ position: "absolute", inset: `${i * -20}px`, borderRadius: "50%", border: "1px solid rgba(180,140,60,0.3)" }} />
          ))}
          <div style={{ position: "absolute", inset: "40px", borderRadius: "50%", background: "rgba(180,140,60,0.1)", border: "1px solid rgba(180,140,60,0.4)" }} />
        </div>
